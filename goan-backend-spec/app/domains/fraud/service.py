...
def is_anomalous_ratio(driver_ratio: float, system_avg_ratio: float, *, factor: float = 2.0) -> bool:
...
async def verify_driver_selfie(
    ...
    db.add(
        FraudIncident(
            ...
            severity=FraudSeverity.ACCOUNT_LOCKED,
            penalty_amount=Decimal("0"),
            details={"match_score": result.match_score, "threshold": settings.SELFIE_MATCH_THRESHOLD},
        )
    )
