...
from sqlalchemy import Date, DateTime, Enum, ForeignKey, Index, JSON, String, Uuid, func
...
class Payment(Base):
    ...
    method: Mapped[PaymentMethod] = mapped_column(
        Enum(PaymentMethod, name="payment_method"), default=PaymentMethod.IN_APP_CARD, nullable=False
    )
...
class ReconciliationReport(Base, TimestampMixin):
    ...
    total_driver_payout: Mapped[Decimal] = mapped_column(Money, default=Decimal("0"), nullable=False)
    total_wallet_credit: Mapped[Decimal] = mapped_column(Money, default=Decimal("0"), nullable=False)
    total_escrow_accrual: Mapped[Decimal] = mapped_column(Money, default=Decimal("0"), nullable=False)
